...
// v3.3.51: [^)]* (첫 ')'에서 멈춤) 대신 그리디 .* 사용 — 조선소명 등에 괄호가 중첩된
// 경우(예: "SHIPYARD (ULSAN) CO LTD") 예전 방식은 안쪽 ')'에서 멈춰 바깥쪽 ')'가 잘린
// 채로 "..., 1453)"처럼 텍스트 잔재가 남았음. 그리디 매칭은 첫 '('부터 문자열의 마지막
// ')'까지를 통째로 제거해 중첩 여부와 무관하게 항상 깨끗하게 지운다.
function _stripShipParen(ship) {
  return (ship || '')
    .replace(/\(.*\)/g, '')     // 첫 '('부터 마지막 ')'까지(중첩 괄호 포함) 통째로 제거
